...
Current slice: Getting started, shared header/select/table/prose presentation, eight-entry current navigation, its checked toggle module and generated export. The same checked toggle also appears in the root and facade READMEs; their broader review remains queued. Validation evidence: [Getting started receipt](getting-started.md) and [shared-theme correction](theme-correction.md). Latest first-page copy/example changes: [onboarding simplification](onboarding-simplification.md). The Operator has explicitly kept the first page unapproved; Sources remains queued.
