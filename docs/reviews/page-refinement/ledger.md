...
This ledger carries all A01–A12 and E01–E10 findings from the supplied audits. Findings outside the current slice are queued reports, not newly verified conclusions. Shared presentation affects all routes; it does not confer content acceptance. No entry is independently accepted. Review order: Getting started, Sources, Views, Events, Ownership, Testing, API, Examples, then remaining destinations individually. Prior review records remain historical.

Current slice: Getting started, shared header/select/table/prose presentation, eight-entry current navigation, its checked toggle module and generated export. The same checked toggle also appears in the root and facade READMEs; their broader review remains queued. Validation evidence: [Getting started receipt](getting-started.md) and [shared-theme correction](theme-correction.md). First-page tooling simplification: [onboarding simplification](onboarding-simplification.md). Independent-instance correction: [toggle machine](toggle-machine.md). Live-example implementation: [light switch](light-switch.md). Whole-page review and correction: [copy and code-block polish](page-polish.md). Shared presentation correction: [native prose width](prose-width.md). Latest first-page readability refinement: [sentence paragraphs](sentence-paragraphs.md). The Operator has explicitly kept the first page unapproved; Sources remains queued.
...
| --- | --- | --- | --- | --- |
| [Getting started](https://0xjcf.github.io/ignite-element/) | A01, A08, A10 | `index.mdx` | awaiting review | Use source → states → view. Shorten installation prose; move alternative JSX configuration into a labeled JSON block or secondary note. Format the HTML entry with one element per line. Keep ownership guidance accurate. |
| [Sources](https://0xjcf.github.io/ignite-element/handbook/sources/) | A01, A08 | `handbook/sources.mdx` | queued | Replace “Native snapshot” with “What states receives”; distinguish store state, MobX toJS output, and Actor-Web extended state. Keep import paths readable in the adapter table. |
...
| Eight-entry beta navigation | A11 | `docs/site/astro.config.mjs` | awaiting review | Archive navigation remains separate. |
| Canonical toggle | A10 | `scripts/__tests__/fixtures/handbook/toggle.tsx` | awaiting review | Unchanged; strict public consumer and runnable bootstrap verification. |
| Current agent exports | A01, A08, A10 | `docs/site/scripts/generate-agent-docs.mjs` → `dist/llms-full.txt`, `llms.txt`, `llms-small.txt` | awaiting review | Regenerate Getting started; other page findings remain queued in the same export. |
