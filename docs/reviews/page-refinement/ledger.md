...
Current slice: Getting started, shared header/select/table/prose presentation, eight-entry current navigation, its checked toggle module and generated export. Unchanged toggle bytes also appear in the root and facade READMEs; their broader review remains queued. Validation evidence: [Getting started receipt](getting-started.md).
...
| [Support & links](https://0xjcf.github.io/ignite-element/community/) | A12 | `community/index.mdx` | queued | Keep the short page. Link examples and changelog directly rather than leaving repository paths as inline code. Use the same footer link colors as the rest of the theme. |
