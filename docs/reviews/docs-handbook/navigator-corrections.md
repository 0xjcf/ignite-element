...
One focused correction commit; no history rewrite. Task-local installs/generated outputs remain retained under the manifest. The strict run was repeated to prove red then green; docs checks were run after the corrections. Normal pre-push runs the full repository profile using the previously verified Node/pnpm launcher environment and canonical task-local TMPDIR; no test or hook is bypassed. GitHub CI is separate fresh evidence. Navigator re-review remains the acceptance boundary for PR #111.
