...
| Ownership | Apply shared retention until `core.dispose()` and rejection of every explicit `cleanup` value. Separate automatic independent runtime cleanup, shared view unsubscription, core disposal and application-owned native shutdown. Explicit headless access stays separate from independent views. Explain retained Activity state/resources, paused view delivery and cleanup on actual removal, plus descendant layout/ref command readiness. |
