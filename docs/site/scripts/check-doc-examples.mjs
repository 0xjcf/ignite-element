 ...
 *
 * No build step needed (it reads package source directly). Run `--update-baseline`
...
const DT = (p) =>
	relative(TMP, join(REPO_ROOT, "packages", p))
		.split(sep)
		.join("/");
...
const COMPILER_OPTIONS = {
	...
	types: [],
	baseUrl: TMP,
	// Map to package SOURCE (mirrors the examples' tsconfig). Mapping to
	// dist/types instead makes TS recurse into pathological generic relations
	// and stack-overflow, so we use src + filter the one src/dist dual-identity
	// artifact it produces (see DUAL_IDENTITY below).
	paths: {
		"ignite-element": [DT("ignite-element/src/index.ts")],
		"ignite-element/jsx": [DT("ignite-element/src/jsx/index.ts")],
		"ignite-element/jsx/jsx-runtime": [
			DT("ignite-element/src/jsx/jsx-runtime.ts"),
		],
		"ignite-element/jsx/jsx-dev-runtime": [
			DT("ignite-element/src/jsx/jsx-dev-runtime.ts"),
		],
		"ignite-element/xstate": [DT("ignite-element/src/xstate.ts")],
		"ignite-element/redux": [DT("ignite-element/src/redux.ts")],
		"ignite-element/mobx": [DT("ignite-element/src/mobx.ts")],
		"ignite-element/actor-web": [DT("ignite-element/src/actor-web.ts")],
		"ignite-element/*": [DT("ignite-element/src/*")],
		"@ignite-element/core": [DT("ignite-core/src/index.ts")],
...
// The src mapping occasionally resolves the same JSX type via both a package's
// `src/` and its `dist/types/`, producing a spurious "X is not assignable to X"
// on every component() call. Such a diagnostic uniquely names both paths.
const DUAL_IDENTITY = (msg) =>
	msg.includes("/dist/types/") && msg.includes("/src/");
...
async function main() {
	if (!existsSync(join(REPO_ROOT, "packages/ignite-element/src/index.ts"))) {
		console.error(
			"[check-doc-examples] Cannot find packages/ignite-element/src — run from the repo.",
		);
		process.exit(2);
	}
