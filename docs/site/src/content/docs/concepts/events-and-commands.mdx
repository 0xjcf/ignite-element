...
- Emit DOM events for the host app.
...
`commands` also receive `host` (the custom element instance) and your adapter’s control surface (`actor`, `store`, or `observable`). Keep commands small and deterministic—send events and emit DOM events; IO belongs in adapters, not commands.
