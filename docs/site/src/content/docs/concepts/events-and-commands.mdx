...
With `events` defined, `effects` receives an `emit` helper:
...
effects: (_snapshot, _prevSnapshot, { emit, select }) => {
  ...
  emit('incremented', { amount: count.current });
},
```
...
effects: (_snapshot, _prevSnapshot, { host, emit }) => {
...
const component = igniteCore({
  ...
  effects: (_snapshot, _prevSnapshot, { emit, select }) => {
    ...
    if (!count.changed) return;
    emit('incremented', { count: count.current });
  },
});
