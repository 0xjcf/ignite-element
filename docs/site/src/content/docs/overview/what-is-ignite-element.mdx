...
- **Source snapshots** expose the source's native state truth.
- **States** derive consumer/read-model data from each native snapshot.
- **Effects** express consequences from state transitions.
- **Events** define the outward contract when you want typed DOM signals.
- **Renderers** own the view and decide how templates are produced: Ignite JSX (default) or lit.

The ownership chain is `source → native snapshot → derived states → renderer view`. Source-backed construction returns a component factory with typed `states`, ordinary `commands`, `effects`, and `events`, plus the `get`, `watch`, `on`, `execute`, and owning `dispose` runtime. Root source-free construction remains registrar-only.
...
- Typed commands, derived states, renderer views, effects, events, and headless-runtime hooks without a React/Solid runtime.
