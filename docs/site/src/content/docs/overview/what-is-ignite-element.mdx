...
The end result is a web component factory with typed `view`, `commands`, `effects`, and `events`, plus a headless runtime for `execute()`, `getSnapshot()`, `getStates()`, and `getSchema()`.
