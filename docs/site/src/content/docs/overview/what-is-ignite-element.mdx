...
The ownership chain is `source → native snapshot → derived states → renderer view`. The result is a web component factory with typed `states`, `commands`, `effects`, and `events`, plus a headless runtime for `execute()`, `getSnapshot()`, `getStates()`, and `getSchema()`.
