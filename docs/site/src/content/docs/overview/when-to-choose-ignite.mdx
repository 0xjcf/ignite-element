...
- `getSchema()` exposes commands, declared events, snapshot, and derived states as JSON-serializable data.
...
- `on(...)` and `watchStates(...)` let tools observe public events and derived states over time.
