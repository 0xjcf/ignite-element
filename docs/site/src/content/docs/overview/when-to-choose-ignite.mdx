...
- You need a headless runtime that agents, tests, and wrappers can drive through `execute()`, `getView()`, `getSchema()`, and `canExecute()`.
...
- `view(...)` projects state for renderers and tools.
...
- `on(...)` and `watchView(...)` let tools observe public events and projected state over time.
