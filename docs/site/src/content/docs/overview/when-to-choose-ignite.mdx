...
- You want commands, derived states, effects, renderer views, and typed public events to stay explicit and inspectable.
- You need a headless runtime that agents, tests, and wrappers can drive through `execute()`, keyed `get`, `watch`, `on`, and owning `dispose()`.
...
- `get('schema')` exposes immutable minimal discovery: actual bound command names and declared event names, with unknown schemas represented as null.
- Public derived states carry source-native availability hints; explicit tool schemas and predicates remain application-owned, and the source enforces behavior.
- `execute()` runs the command and returns the resulting snapshot, derived states, and emitted events.
- `on(...)` and `watch(...)` let tools observe public events and derived states over time.
...
- Read [Headless runtime](/ignite-element/api/headless-runtime/#discovery) for the exact `get('schema')` contract.
