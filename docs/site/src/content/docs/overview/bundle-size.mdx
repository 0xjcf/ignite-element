...
These retained numbers measure the pre-core-bindings public `ignite-element` ESM entrypoints after a
...
code a host app pulled in through each import. They are historical, not measurements
of the core-API/bindings candidate. In particular, the old React row measured the
web wrapper now relocated to `/react/web`, not the neutral `useIgnite` hook.
