---
title: Historical bundle measurements
description: Historical measurements, not current release bundle sizes.
sidebar:
  label: Bundle size
  order: 5
---

These retained numbers measure the pre-core-bindings public `ignite-element` ESM entrypoints after a production Vite build.

State-library and framework peers are externalized, while Ignite's own workspace packages are bundled so the table reflects the Ignite code a host app pulled in through each import.

They are historical, not measurements of the current release.

In particular, the old React row measured the web wrapper now relocated to `/react/web`, not the neutral `useIgnite` hook.
...
- **Peers are not counted**: XState, Redux Toolkit, Redux, MobX, React,
  and `lit-html` remain host-app dependencies.
...
These are full-entrypoint measurements.

A host app that imports a smaller subset of symbols can be smaller after its own bundler finishes tree-shaking.
