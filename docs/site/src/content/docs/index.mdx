...
The verified public release is `3.0.0-beta.14`. It adds [native event forwarding and core-owned effects](/ignite-element/guides/events/), retaining beta.13's shared readiness and terminal disposal. Install that exact version or follow the beta channel; stable `ignite-element@latest` remains v2.
...
The source-backed `toggle` core is usable headlessly before registration, without browser globals. Registering `<ignite-toggle>` requires a browser DOM; keep modules that register tags during evaluation on the browser side.
...
Every press is a command — the buttons express intent, the state machine owns the transition, `states(snapshot)` derives the read model, and the renderer view re-renders from those states. Here is the same counter with the real API:
...
- [Open repo example](https://github.com/0xjcf/ignite-element/tree/beta/examples/adapters/xstate)
