...
This page previews the next v3 beta. Published beta.14 does not support the `{ source }` command context shown here.

The live demo uses the local candidate. The installation and download instructions below require the upcoming release.
:::
...
After the next beta is published, install Ignite and XState:
...
After the next beta is published, unzip it and run `pnpm install` and `pnpm dev` from its directory.
