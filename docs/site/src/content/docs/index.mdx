...
Try the counter directly in the docs, or open the full example in StackBlitz. The card below is the same `<my-counter>` mental model from the example above: buttons dispatch intent events, the state owns the transition, and the view re-renders from state.
