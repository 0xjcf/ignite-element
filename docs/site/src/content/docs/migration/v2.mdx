...
5. Verify lifecycle: shared adapters are released when the last element disconnects unless `cleanup: false`.
...
Most projects should not need to touch `cleanup`; treat it as an override.
