...
5. Verify shared-adapter lifetime: a consumer-owned shared adapter remains observed until terminal core disposal. The local lifecycle candidate removes `cleanup`; see the [migration note](/ignite-element/migration/remove-cleanup/).
...
A consumer-owned shared source (a started actor/store/observable you pass) is kept alive for the core's lifetime — element disconnects don't release its adapter, and Ignite never stops a source it didn't create. Isolated sources Ignite creates (e.g., a bare XState machine) still get a per-element adapter stopped on disconnect.
The local lifecycle candidate rejects any explicitly supplied `cleanup` key. Published beta.14 still exposes the option; its early-teardown path can freeze reconnects.
