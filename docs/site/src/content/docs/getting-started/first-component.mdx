...
- Opt into the lit renderer only if you need it, via the advanced config/docs path.
- Share actors/stores by passing long-lived instances. Ignite never stops user-owned sources.
- Advanced overrides: use `cleanup` or `lifecycle.browser` only when you need to change the defaults.
