...
For the default path, you do not need `ignite.config.ts`, a bundler plugin, or lifecycle overrides. The default Ignite JSX renderer is registered automatically by the public entrypoints.

### Optional advanced setup

Use the advanced setup only when you need project-wide renderer defaults, global styles, diagnostics, or a non-default renderer.

- **Config**: [defineIgniteConfig](/ignite-element/api/define-ignite-config/)
- **Tooling plugins**: [Tooling](/ignite-element/guides/tooling/)
- **lit renderer**: [Renderers](/ignite-element/api/renderers/)
