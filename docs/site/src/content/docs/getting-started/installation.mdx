...
- **Advanced setup (optional)**: reach for [`ignite.config.ts`](/ignite-element/api/advanced-config/) only when you need project-wide renderer defaults, shared styles, diagnostics, or the legacy `lit` renderer.
