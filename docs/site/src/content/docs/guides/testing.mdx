...
For deterministic command/event assertions, use the built-in scenario helper against the headless runtime returned by `igniteCore(...)`.
...
This path is useful when you want to verify command execution, resulting state, and emitted events without mounting a custom element. The helper wraps `execute()`, so the observed events are the exact ones returned by the runtime for that transition.
...
- For Ignite-created sources (e.g., XState machines), you can assert cleanup by connecting/disconnecting elements and verifying the adapter is released when `cleanup` is `true`.
- If you enable `lifecycle.browser`, simulate `visibilitychange` or `pagehide` and assert XState machines receive `SUSPEND`/`RESUME`.
