...
- Use a browser-like runner (e.g., Vitest + JSDOM). If you rely on `ignite.config.ts`, global styles, or a non-default renderer, import that setup once before tests.
...
import 'ignite-element/renderers/lit'; // only if your tests use the lit renderer
