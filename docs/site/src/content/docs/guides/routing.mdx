...
Every page element and the outlet read the **same** router actor. Ignite stops a
shared adapter when the last instance of an element disconnects — and because the
outlet swaps pages, that would tear down the router the incoming page still
needs. Pass `cleanup: false` so the shared adapter stays alive; the shell owns
the actor's lifetime instead.
:::
