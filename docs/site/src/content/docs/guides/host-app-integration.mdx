...
Create one TSX module that registers your elements, then import that module from
the host app entrypoint. Configure its JSX import source as `ignite-element/jsx`:
...
The `Toggle` export from `register-ignite.tsx` above is the registration handle
passed to `igniteReact`; extensionless TypeScript imports resolve that TSX module.
The plain HTML example imports its compiled `.js` output.

`register-ignite.tsx` stays **framework-neutral** — it is just the ignite element.
