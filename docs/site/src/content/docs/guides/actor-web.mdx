...
`get('schema').events` enumerates the names declared in the `events:` map. Source-emitted types are a compile-time union, not runtime-enumerable, so they don't appear there — they surface dynamically via `on(...)` and `execute().events`. See the [`get('schema')` caveat](/ignite-element/api/headless-runtime/#getschema).
:::
