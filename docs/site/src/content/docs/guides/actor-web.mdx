...
export const shipmentBadge = igniteCore({
  ...
  states: (snapshot) => ({
    ...
    degradedReason:
      snapshot.transport.state === "degraded"
        ? (snapshot.transport.reason ?? null)
        : null,
  }),
});
```
...
const result = await shipmentFeed.execute({
  command: "cancel",
  input: "shipment-1",
});
...
See [Choosing `xstate` vs `actor-web`](/ignite-element/handbook/sources/#optional-integrations) for the boundary rationale, and [Redux & MobX](/ignite-element/handbook/sources/) for the other two adapters.
