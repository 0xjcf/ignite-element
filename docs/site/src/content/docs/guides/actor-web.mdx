...
```ts
import { homeTopology, projectHomeView, startRuntime } from './application';
...
try {
  ...
  // Mount or drive `home` while the Actor-Web runtime remains live.
} finally {
