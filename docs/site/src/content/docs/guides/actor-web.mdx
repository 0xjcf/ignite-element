...
Each projection snapshot carries transport metadata describing the runtime connection. Opt into it explicitly through the `transport` field on the `states`/`view` callback — most components should stay focused on business state.
