...
* The Vite and webpack config plugins (`igniteConfigVitePlugin` from `ignite-element/config/vite` / `IgniteConfigWebpackPlugin` from `ignite-element/config/webpack`) inject the config loader, which in turn auto-imports the renderer you set in `ignite.config` (including `lit`). If you are not using those plugins, import the renderer entry yourself before registering components, e.g. `import "ignite-element/renderers/lit"`.
