...
Keep `ignite.config.ts`, `vitest.config.ts`, and the `counter.*` files below at
the project root; only `setup.ts` lives in the `test/` directory. Each named
file is a separate module.

```ts
// test/setup.ts
import '../ignite.config'; // the config is at the project root
