...
Declare `events` to type `emit`, then fire DOM `CustomEvent`s from your commands. Prefer names with casing that maps to Ignite JSX handler props (`onEventName`). See [Events & commands](/ignite-element/2.x/concepts/events-and-commands/) for more patterns.
