...
Call `igniteTest(component)` with a runtime to get a chainable scenario. `given(...)` asserts the starting snapshot, `when(...)` executes a command (it is async — `await` it), and the `expect*` methods assert the result of the most recent `when(...)`.

```ts
(await igniteTest(counter).given({ value: 'off' }).when('toggle'))
...
Snapshot, view, and payload expectations accept a deep-partial object or a predicate function.
