...
Call `igniteTest(component)` with a runtime to get a chainable scenario. `given(...)` asserts the starting snapshot, `when({ name, input? })` executes a command (it is async — `await` it), and the `expect*` methods assert the result of the most recent `when(...)`.

```ts
(await igniteTest(counter).given({ value: 'off' }).when({ name: 'toggle' }))
...
- `when({ name, input? })` — execute a command; `input` is required for one-argument commands, omitted for no-argument commands, and optional when the command's first argument is optional. Returns a `Promise` of the scenario.
...
- `execute(commandName, payload?)` — run a command and record it; resolves to the same `{ snapshot, events }` as the runtime.
