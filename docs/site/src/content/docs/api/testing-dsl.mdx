...
When you need more than one intent in the same falsifiable user story, use `narrative(name, async (narrative) => ...)`. It reuses the runtime's existing `record(name)` story and returns the same serializable `IgniteStorySnapshot` that `snapshotStory(story)` produces — there is no second recorder or alternate trace schema.
...
- `narrative(name, run)` — create a named story, run multiple ordered command steps, and return the existing `{ name, trace, lifecycle, summary }` snapshot.
