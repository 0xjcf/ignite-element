...
Plain functions are valid commands. When an agent or inspector needs more context than a name, wrap a command with `command(fn, metadata)` to attach a JSON-serializable description and input schema, plus an optional runtime availability predicate. The serializable metadata surfaces in [`getSchema()`](/ignite-element/api/headless-runtime/#getschema) without changing how the command is called.
...
- `canExecute?: ({ snapshot }) => boolean` — current-snapshot availability for
  the [headless runtime](/ignite-element/api/headless-runtime/#canexecutecommandname).
...
`canExecute` lets a command declare its current availability without moving guard
...
The predicate function is not serialized into `getSchema()`. Instead the schema
...
`getSchema().commands` is a JSON-serializable map of command metadata, not callable functions. Execute commands through `runtime.execute({ command, input? })`. Commands without metadata appear as empty objects:
...
// getSchema() returns:
{
  ...
  events: [{ type: 'limit-reached' }],
  snapshot: { value: 'active', context: { count: 0, limit: 6 } },
  states: { count: 0, limit: 6 },
}
```
