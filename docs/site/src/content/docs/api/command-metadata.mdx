...
Returns the original command function, tagged with non-enumerable metadata. Calling the command behaves exactly like calling `fn`; the wrapper only adds context for `getSchema()`. Passing no `metadata` returns the function unchanged.
