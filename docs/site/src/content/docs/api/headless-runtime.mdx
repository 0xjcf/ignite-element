...
**Returns** `{ commands, events, snapshot, view }` — a JSON-serializable `IgniteAgentSchema`: `commands` maps each name to its metadata (`{}` when none), `events` is the array of declared event names, `snapshot` is the current snapshot value, and `view` is the projected view. Commands with an availability predicate include `gated: true`; call `canExecute(commandName)` for the current boolean result.
