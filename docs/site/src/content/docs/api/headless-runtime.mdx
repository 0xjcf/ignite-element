...
Returns the current raw state snapshot of the underlying source.
...
Subscribes to **raw state** updates. The handler receives `(state, prevState)` on every source update.
