...
const { snapshot, events } = await counter.execute('toggle');
...
2. Use `input` to shape payloads before calling `execute(commandName, payload?)`.
...
const result = await panel.execute('armAlarm');
