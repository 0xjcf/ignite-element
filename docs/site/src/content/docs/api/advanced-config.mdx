...
Using lit is **not** on this list anymore: it is config-free — see [Rendering](/ignite-element/concepts/rendering/).

## The `ignite.config.ts` file

`ignite.config.ts` is an ordinary TypeScript module that calls `defineIgniteConfig` and `export default`s the result. There is **no auto-loader** in the supported v3 beta API — you wire the file in yourself.
...
See [Rendering](/ignite-element/concepts/rendering/) for the full authoring and selection story.
...
- Legacy config-loader plugins are compatibility-only internals, not part of the public v3 beta API. Keep them only in setups you already own.
