...
Using lit is **not** on this list anymore: it is config-free — see [Rendering](/ignite-element/handbook/views/).

## The `ignite.config.ts` file

`ignite.config.ts` is an ordinary TypeScript module that calls `defineIgniteConfig` and `export default`s the result. There is **no auto-loader** in the v3 beta API — you wire the file in yourself.
...
**3. Bundler integration.** Vite, Webpack, esbuild, and similar bundlers resolve `ignite.config.ts` as a normal module — no plugin is required. Because the side effect runs at import time, keep the import at the **top** of your entry. The default JSX path needs only the TypeScript setting; see [Installation](/ignite-element/) for `jsxImportSource` wiring.
...
Because this path imports a scoped renderer directly, `@ignite-element/renderer` must be a direct dependency. Normal facade/JSX consumers do not need to install scoped packages explicitly.
