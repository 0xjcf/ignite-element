...
Returning the same existing observable would share its state.
...
In the local lifecycle candidate, independent hook construction, including factory middleware/enhancers, must be safe to repeat and discard without cleanup; see [ownership](/ignite-element/handbook/ownership/).
...
`source: () => new Counter()` lets Ignite create a new instance for each element, or each hook in the local lifecycle candidate.
