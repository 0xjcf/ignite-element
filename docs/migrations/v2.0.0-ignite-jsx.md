...
5. **Verify lifecycle** – shared adapters now stay alive until the host application stops them. A reference-count helper is on the roadmap; until then, manage shutdown in your host app.
...
- Shared adapters (long-lived instances you provide) remain running until you stop them. Manual cleanup is still recommended in v2. A reference-count helper is planned (see the v2 roadmap).
...
- Watch the changelog for follow-up improvements (shared adapter reference counting, lit strategy enhancements).
...
- [ ] Confirmed shared adapter lifecycle is managed by the host app
