...
- **lit strategy (optional)** – keep existing lit templates by setting `renderer: "lit"` in `ignite.config.ts`. The provided config plugins will import the lit strategy automatically; if you aren’t using them, add `import "ignite-element/renderers/lit"` near your config.
