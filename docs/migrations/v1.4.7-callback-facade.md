...
| Render function receives `{ state, send }` | Render function receives facade values + `send` |
...
Render arguments now contain the facade result and the `send` helper. You no longer need to reach into `args.state`:
...
- Let TypeScript infer facade types directly from the callbacks you provide.
