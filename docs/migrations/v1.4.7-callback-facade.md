...
- Let TypeScript infer façade types: `type CounterFacade = AdapterPack<typeof component>;`
- If you need the raw adapter, keep using the second argument inside `commands`.
- `AdapterPack` is optional—render args always include the derived values you return.
...
- Watch the changelog for the upcoming configuration API (`ignite.config.ts`) that will streamline global styling.
