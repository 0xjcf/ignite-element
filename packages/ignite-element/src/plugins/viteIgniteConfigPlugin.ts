...
export function igniteConfigVitePlugin(
	...
	const transformIndexHtml = () => {
		...
		// When running inside the workspace or when the helper is bundled next to this plugin
		// (as it is in the published package), derive a relative path from the plugin file.
		const pluginAdjacentHelperTs = fileURLToPath(
			new URL("../config/loadIgniteConfig.ts", import.meta.url),
		);
		const pluginAdjacentHelperJs = fileURLToPath(
			new URL("../config/loadIgniteConfig.js", import.meta.url),
		);

		const helperPath =
			(distHelperPath && existsSync(distHelperPath) && distHelperPath) ||
			(existsSync(pluginAdjacentHelperJs) && pluginAdjacentHelperJs) ||
			(existsSync(pluginAdjacentHelperTs) && pluginAdjacentHelperTs);

		const loadHelperSpecifier = helperPath
			? toFileSystemPath(helperPath)
