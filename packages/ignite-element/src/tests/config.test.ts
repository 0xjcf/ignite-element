...
describe("defineIgniteConfig", () => {
	...
	it("invokes setGlobalStyles when styles are provided", () => {
		...
		expect(globalStylesModule.getGlobalStyles()).toBe(styles);
	});

	it("does not invoke setGlobalStyles when styles are omitted", () => {
		defineIgniteConfig({});

		expect(globalStylesModule.getGlobalStyles()).toBeUndefined();
	});
	...
	it("supports absolute styles URLs (e.g., CDN assets)", () => {
		...
		expect(globalStylesModule.getGlobalStyles()).toBe(cdnStyles);
	});
	...
	it("supports absolute StyleObject hrefs", () => {
		...
		expect(globalStylesModule.getGlobalStyles()).toEqual(styleObject);
	});
