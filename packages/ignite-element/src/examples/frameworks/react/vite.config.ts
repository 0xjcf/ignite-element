...
export default defineConfig({
	...
	resolve: {
		alias: [
			...
			// The explicit `ignite-element/react` alias MUST precede the generic
			// `ignite-element/(.+)` regex, or the regex would rewrite it to
			// `${root}react` (the bare dir) and miss the entrypoint module.
			{
				find: "ignite-element/react",
				replacement: resolvePath("../../../react/index.ts"),
			},
			{
				find: /^ignite-element\/(.+)$/,
