# Implement the optional selected-specimen XState observation lens for Ignite Alchemy

## Source
Created with `fas create-task` on 2026-07-20.

## Problem
Implement an optional XState topology and observation lens for the currently selected Ignite Alchemy specimen without changing Story execution or requiring every specimen to own a machine. Derive topology with XState-owned utilities and emit monotonic ordered evidence for real active states, snapshot deltas, directly evidenced or candidate edges, child or passive activity, context changes, and explicitly unknown causality. Support component, feature projection, and integration specimens, including multiple specimens observing one shared app-owned actor; never introduce a machine-per-visual-atom rule or duplicate actor authority. Bound and redact raw context evidence, fail closed on trigger and guard certainty, and preserve complete no-lens operation.
...
- A selected specimen may have no machine, and the lens never requires a machine for each component, feature projection, DOM node, or visual atom.
- Multiple selected specimens may observe one shared app-owned actor through separate read-only projections without creating, wrapping, or duplicating that actor.
...
## Proposed solution
- Install optional XState actor-system inspection for the selected specimen through the W2 fixture hook before actor start, and separately derive static topology using XState-owned graph/config utilities.
...
- Rejected machine-required specimen admission, a machine per visual atom, or duplicating a shared app-owned actor to isolate each specimen.
...
- Lens subscriptions could alter lifecycle, duplicate a shared actor, or retain actors if attachment and disposal are not exact.
...
- Blocks repo-scoped specimen workbench host task-1784602901002 together with the selected-specimen Story session controller.
