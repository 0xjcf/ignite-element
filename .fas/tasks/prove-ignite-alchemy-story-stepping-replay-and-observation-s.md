# Prove Ignite Alchemy Story stepping, replay, and observation seams in a browser POC

## Source
Created with `fas create-task` on 2026-07-21.

## Problem
Use the approved Ignite Alchemy prototype and browser-safe shared Voice Workbench Stories to retire the smallest risky integration seams before MVP implementation. Build a deliberately narrow browser POC around the real timeout and retry Story, the existing igniteTest component story executor, controlled ports, exactly-once page gates, fresh-fixture Back replay, ordinary final Story receipts, and optional XState actor-system observation installed before actor start. Prove ordered active-state and candidate-edge evidence without adding a public Ignite API, second runner, graph algorithm, state authority, or trace. Keep POC evidence and source explicitly separate from production-readiness claims and record whether each part should be retained, rewritten, or retired.

## Acceptance criteria
- A browser-safe POC imports the shared timeout and retry Story and executes it through the existing igniteTest component story API against the real Voice Workbench machine and controlled fixture ports.
...
- XState observation is installed at actor creation and proves ordered observations, all active parallel regions, snapshot deltas, and uniquely evidenced or candidate topology edges while unknown causality fails closed.
- The approved MagicPath layout direction is represented only enough to exercise the risky interactions; visual polish, seven-Story completeness, coverage indexing, report generation, and production packaging remain outside the POC.
...
## Proposed solution
- Add a temporary Vite POC entry that imports the W2 timeout/retry Story, constructs one fresh real fixture per session, decorates the existing Story context with gates, and installs optional XState inspection before actor start.
- Implement only enough approved Ignite Alchemy shell to exercise Step, Cancel, Restart, Back replay, active parallel nodes, snapshot deltas, candidate evidence, and the final unchanged receipt.
...
- Rejected building the complete application, all seven Stories, bidirectional coverage, final report, or production design polish inside the POC.
...
## Dependencies
- Depends directly on approved prototype task-1784655399770.
