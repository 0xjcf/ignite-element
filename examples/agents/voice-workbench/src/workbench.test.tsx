...
describe("voice workbench accessible JSX", () => {
	...
	it("renders the approved empty-to-artifact workflow from the component view", async () => {
		component("voice-workbench-accessibility", renderWorkbench);
		const host = document.createElement("voice-workbench-accessibility");
		...
		try {
						...
						rows: [
							{
								...
								schemaText: expect.stringContaining(
			...
			expect(host.shadowRoot?.textContent).toContain("0 turns");
			expect(host.shadowRoot?.textContent).toContain("0 artifacts");
			expect(host.shadowRoot?.textContent).not.toContain("browser-demo");
			expect(host.shadowRoot?.textContent).toContain("MLX model readiness");
			expect(host.shadowRoot?.textContent).toContain("Compound actor state");
			expect(host.shadowRoot?.textContent).toContain("Capability outcomes");
			expect(host.shadowRoot?.textContent).toContain("Retry budget exhausted.");
			const policyProof = host.shadowRoot?.querySelector(
			...
			const clarificationQuality = host.shadowRoot?.querySelector(
			...
			expect(
				queries.getByRole("button", { name: "Browser preview" }),
			).toBeTruthy();
			expect(
				queries.getByRole("button", { name: "Terminal preview" }),
			).toBeTruthy();
			expect(
				queries.getByRole("button", { name: "Speech preview" }),
			).toBeTruthy();
			expect(
			...
			expect(host.shadowRoot?.textContent).toContain("All-component blueprint");
			const searchSchema = host.shadowRoot?.querySelector(
			...
			expect(host.shadowRoot?.querySelector(".actor-match")?.textContent).toBe(
				'matches({\n  available: { turn: "idle" },\n})',
			);
			...
			expect(host.shadowRoot?.textContent).toContain("1 turn");
			expect(host.shadowRoot?.textContent).not.toContain("1 turns");
			...
			expect(host.shadowRoot?.textContent).toContain("Text prompt");
			expect(host.shadowRoot?.textContent).toContain("Show the decision");
			expect(host.shadowRoot?.textContent).toContain(
				"Ignite owns the projection.",
			);
			expect(host.shadowRoot?.textContent).toContain("decision · revision 1");
			expect(host.shadowRoot?.querySelectorAll("[data-node-kind]").length).toBe(
				9,
			);
			expect(
				Array.from(
					host.shadowRoot?.querySelectorAll("[data-node-kind]") ?? [],
			...
			expect(
				queries.getByRole("textbox", { name: "Owner team" }),
			).toHaveProperty("readOnly", true);
			expect(host.shadowRoot?.querySelector("table")?.textContent).toContain(
				"Hosting",
			);
			const sourceLink = queries.getByRole("link", {
			...
			expect(
				host.shadowRoot?.querySelector('[aria-label="Timeline"]')?.textContent,
			).toContain("Launch");
			expect(
				host.shadowRoot?.querySelector("progress")?.getAttribute("aria-label"),
			).toBe("Budget used: 40");
			expect(
				host.shadowRoot
			...
			expect(
				host.shadowRoot?.querySelector('[aria-label="Decision log"]')
					?.textContent,
			).toContain("Keep the actor authoritative");
			expect(
				host.shadowRoot?.querySelector('[aria-label="Artifacts"]')?.textContent,
			).toContain("Decision");
			...
			const action = queries.getByRole("button", { name: "Complete response" });
			const pendingChecklistItem = queries.getByRole("checkbox", {
				name: "Ship Ignite",
			}) as HTMLInputElement;
			...
			expect(
				host.shadowRoot?.querySelector('[aria-label="Revision history"]')
					?.textContent,
			...
			const secondPrompt = queries.getByRole("textbox", { name: "Prompt" });
			...
			expect(
				host.shadowRoot?.querySelector(".capability-outcomes")?.textContent,
			...
			expect(
				host.shadowRoot?.querySelector(".collision-proof")?.textContent,
			...
			const shopperQuality = host.shadowRoot?.querySelector(
				'[aria-label="Shopper result quality"]',
			);
			...
			expect(
				host.shadowRoot?.querySelector(".document h1")?.textContent,
			).toContain("Shopping List Whole Foods");
			expect(
				host.shadowRoot?.querySelector('[data-node-kind="table"]')?.textContent,
			...
			expect(
				queries
					.getAllByRole("link", { name: "Source: wholefoodsmarket.com" })
					.map((link) => link.getAttribute("href")),
			).toEqual([
				"https://www.wholefoodsmarket.com/product/eggs",
				"https://www.wholefoodsmarket.com/product/milk",
			]);
			const admittedPolicyProof = host.shadowRoot?.querySelector(
				'[aria-label="Domain policy proof"]',
			);
			...
			expect(admittedPolicyProof?.textContent).toContain(
