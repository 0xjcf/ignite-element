	...
	>
		<div class="artifact-toolbar">
			<div class="artifact-identity">
				<strong>{context.activeArtifact?.title ?? "Artifact workspace"}</strong>
				<span>
			...
			<div class="segmented" role="tablist" aria-label="Artifact view">
				<button
					...
					type="button"
					data-view="document"
					aria-selected={context.presentation.artifactView === "document"}
		...
		<div class="artifact-scroll">
			...
			<div class="proof-banner">
				...
				<div>
					...
					<span>
						Accepted text or speech commands update this center artifact, its
						schema, and every commit receipt.
					</span>
				</div>
			</div>
			{!context.activeArtifact &&
			(context.modelPreparing || context.modelFailed) ? (
				...
				<article class="document" data-artifact-document="">
					<div class="doc-kicker">
						Actor-owned artifact · revision {context.activeArtifact.revision}
					</div>
					<h1>{context.activeArtifact.displayTitle}</h1>
					{context.activeArtifactRevisions.length > 1 ? (
						<section class="revision-history" aria-label="Revision history">
				...
				<section class="empty-artifact">
					<div>
						<strong>Your first accepted artifact will appear here</strong>
						<p>
							The model proposes semantic nodes. The actor validates and stores
							them before Ignite renders anything.
						</p>
					</div>
				</section>
			)}
